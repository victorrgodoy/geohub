...
import { getTotalCity, type CityTotalResponse,listTop5City, type CityResponse } from "../api/city"
import { getTotalCountry, getTotalPopulation, listTop5Country,type CountryTotalResponse, type CountryReponse} from "../api/country"
...
export const useDashboard = () => {
    ...
    const [totalPopulation, setTotalPopulation] = useState<CountryTotalResponse>()
    ...
    const [top5Country, setTop5Country] = useState<CountryReponse[]>([])
     ...
     useEffect(() => {
        const fetchData = async () => {
            const [cityTotal, countryTotal, populationTotal, citiesTop5, countriesTop5] = await Promise.all([
                getTotalCity(),
                getTotalCountry(),
                getTotalPopulation(),
                listTop5City(),
